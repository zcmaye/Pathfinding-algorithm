...
struct Cmp:std::binary_function<const TreeNode*, const TreeNode*,bool>
{
	bool operator()(const TreeNode* item,const TreeNode* node) const
	{
		return (*item) == (*node);
	}
};
...
public:
	...
	{	
		...
		while (true)
		{
			...
			if (num++%1000 == 0)
			{
				printf("num:%d  open_list:%d  close_lsit:%d\n", num,open_list.size(),close_list.size());


			}
			//printf("\n");
			//找出数组中f值最小的点
			std::vector<TreeNode*>::iterator itMin = open_list.begin();
			for (std::vector<TreeNode*>::iterator it = open_list.begin(); it != open_list.end(); it++)
			{
...
private:
	...
	{
		//std::bind(cmp, std::placeholders::_1, node);
		//已经走过了
		bool isok = std::find_if(close_list.begin(), close_list.end(), 
			std::bind2nd(Cmp(),node)
		)!=close_list.end();
		if (isok)
		{
			return false;
